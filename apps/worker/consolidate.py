...
def fetch_new_edge_keys_for_docs(db:Session, *, space_id: str, document_id: str)-> list[str]:
    """ 
    Uses evidence table as the source of truth: edges that have evidence from this document.
    """
    rows = db.execute(
        text(
            """ 
...
def fetch_edge_triple(db: Session, *, space_id: str, edge_key: str) -> tuple[str, str, str, float]:
    """ 
    Return (src_key, relation, dst_key, confidence) from AGE edge by edge_key.
    we depend on how you store src/dst kets; if not stored on edge, we infer from connected vertices.
    """
    db.execute(text(_age_setup_sql()))
    cypher = """ 
    SELECT * FROM cypher(:g, $$
    ...
    rel = str(row[1])
    dst = str(row[2])
    conf = float(str(row[3]))
    return src, rel, dst, conf
...
def consolidate_document(db:Session,*, space_id: str, document_id: str) -> None:
    ...
    for new_ek in new_edge_keys:
        ...
        # for non-attribute relations: choose update if we can else contradict
        # and also we will deprecate older edges if new confidence is >= old confidence
        updated_any = False
        for old_ek in competitors:
            ...
            #  same dst already exists -> just mark new edge active and optionally deprecate duplicate
            if old_dst == dst_key:
                set_edge_state(db, space_id = space_id, edge_key = new_ek, state = "active", valid_from = now, updated_by_doc_id = document_id)
                updated_any = True
                continue
            ...
            if new_conf >= old_conf:
                ...
                # CONTRADICT: cannot safely replace
