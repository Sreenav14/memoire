...
def upsert_vertex(
    ...
    # 1) If we have mapped this key, return its AGE id
    row = db.execute(
...
def upsert_edge(
    ...
    props: dict | None = None,
    *,
    # lifecycle metadata (defaults keep existing behavior)
    kind: str = "explicit",          # explicit | inferred
    state: str = "active",           # active | deprecated | disputed
    ...
    # create edge label as relation (cleaner for traversal)
